...
class AtDataController extends StateNotifier<AsyncValue<List<AtData>>> {
  ...
  Future<void> getData() async {
    ...
    state = await AsyncValue.guard(
        () async => await ref.watch(dataRepositoryProvider).getData());
  }
...
final atDataControllerProvider =
    StateNotifierProvider<AtDataController, AsyncValue<List<AtData>>>(
        (ref) => AtDataController(ref: ref));
...
class FilterController extends StateNotifier<AsyncValue<List<AtData>>> {
  ...
  void getFilteredAtData() {
      ...
      state.value!.where(
        (element) {
          ...
          for (final filterOption in searchFormModel.filter) {
            ...
            final int filterOptionIndex =
                searchFormModel.filter.indexOf(filterOption);
            log(filterOptionIndex.toString());

            var searchContent =
                searchFormModel.searchRequest[filterOptionIndex].toString();
            switch (filterOption) {
              ...
              case Categories.contains:
                ref
                    ...
                    .isConditionMet
                    .add(element.atKey.toString().contains(searchContent));
              ...
              case Categories.dateCreated:
                ...
                final startDate =
                    _getDate(DateTime.parse(searchContent.split(' - ')[0]));
                log(startDate.toString());
                final endDate =
                    _getDate(DateTime.parse(searchContent.split(' - ')[1]));
              ...
              case Categories.dateModified:
                ...
                if (element.atKey.metadata?.updatedAt != null) {
                  if (element.atKey.metadata!.updatedAt!.isAfter(
                          DateTime.parse(searchContent.split(' - ')[0])) &&
                      element.atKey.metadata!.updatedAt!.isBefore(
                          DateTime.parse(searchContent.split(' - ')[1]))) {
                    ref.watch(searchFormProvider).isConditionMet.add(true);
                  } else {
                    ref.watch(searchFormProvider).isConditionMet.add(false);
                  }
              ...
              case Categories.apps:
                if (element.atKey.namespace != null) {
                  ref
                      .watch(searchFormProvider)
                      .isConditionMet
                      .add(element.atKey.namespace!.contains(searchContent));
                } else {
                  ref.watch(searchFormProvider).isConditionMet.add(false);
                }
                break;
              ...
              case Categories.keyTypes:
                if (AtKey.getKeyType(element.atKey.toString()).name ==
                    searchContent) {
                  ref.watch(searchFormProvider).isConditionMet.add(true);
                } else {
                  ref.watch(searchFormProvider).isConditionMet.add(false);
              ...
              case Categories.sharedWith:
                if (element.atKey.sharedWith != null) {
                  ref
                      .watch(searchFormProvider)
                      .isConditionMet
                      .add(element.atKey.sharedWith!.contains(searchContent));
                } else {
                  ref.watch(searchFormProvider).isConditionMet.add(false);
                }
                break;
              case Categories.sharedBy:
                if (element.atKey.sharedBy != null) {
                  ref
                      .watch(searchFormProvider)
                      .isConditionMet
                      .add(element.atKey.sharedBy!.contains(searchContent));
                } else {
                  ref.watch(searchFormProvider).isConditionMet.add(false);
                }
                break;
    ...
    if (sort == 'ascending') {
      state.value!
          .sort((a, b) => a.atKey.toString().compareTo(b.atKey.toString()));
    } else if (sort == 'descending') {
      state.value!
          .sort((a, b) => b.atKey.toString().compareTo(a.atKey.toString()));
    }
  }
}
...
final filterControllerProvider =
    StateNotifierProvider<FilterController, AsyncValue<List<AtData>?>>(
        (ref) => FilterController(ref: ref));
